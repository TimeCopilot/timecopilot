...
class TiRex(Forecaster):
    ...
    def _forecast(
        ...
        fcsts_quantiles_np = None if quantiles is None else np.concatenate(fcsts_quantiles)
    ...
    def forecast(
        ...
        if qc.quantiles is not None and len(qc.quantiles) != len(DEFAULT_QUANTILES_TIREX):
            raise ValueError(
